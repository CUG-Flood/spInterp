...
#' @param width The fixed width of numbers (except for `site` column) in formatted data.
...
anusplin_params <- function(dat,
  ...
  if (is.null(alt)) {
    ...
    cvt.alt <- if (lim.alt == 'auto') {
      ...
      range.dem <- raster(file.alt) %>% values() %>% range(na.rm = T)
      c(
  ...
  if (width == 'auto') {
    width1 <-
      dat[, lapply(.SD, nchar) %>% max(na.rm = T), .SDcols = !'site'] %>%
      max(na.rm = T)
    if (nchar(missing) > width)
      stop(glue(
        'Length of missing values should not be more than the column width ({width})'
      ))
  }
  ...
  lapgrd <- c(
    ...
    '',
    grid.pos,
    ifelse(diff(pos.coord) > 0, 1, 2),
    glue('{range[1]} {range[2]} {res}'),
    ifelse(diff(pos.coord) > 0, 2, 1),
