...
anusplin_params <- function(dat,
  ...
  splina <- listk(
    ...
    sur.spl,
    # Number of surface independent spline variables
    sur.cov,
    # Number of surface independent covariates
    cvt.lon,
    cvt.lat,
    cvt.alt,
    cvt.coef,
    trans.dep = paste(trans.dep, collapse = " "),
    ...
    # Number of surfaces
    err.wgt,
    # Number of relative error variances
    optimize,
    smooth,
  ...
  lapgrd <- listk(
    ...
    type.grd,
    missing,
    # Special value of output grid
    file.out = glue("{names[pos.var]}.grd"),
    # oupput grid file name
